locals {
  vm_name_prefix3 = "az${local.settings.variables.shortregion}${local.settings.variables.onboarddetailsid}ctm${var.short_env[var.environment]}"
...
module "virtual-machine3" {
  ...
  virtual_machine_name = local.vm_name_prefix3
  ...
  virtual_machine_size      = var.vm_sku3
  #admin_username            = "${local.virtual_machine_name}-adm"
  instances_count           = var.vm_single_count3
