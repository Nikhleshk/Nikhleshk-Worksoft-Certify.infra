locals {
  vm_name_prefix2 = "az${local.settings.variables.shortregion}${local.settings.variables.onboarddetailsid}api${var.short_env[var.environment]}"
...
module "virtual-machine2" {
  ...
  virtual_machine_name = local.vm_name_prefix2
  ...
  virtual_machine_size      = var.vm_sku2
  #admin_username            = "${local.virtual_machine_name}-adm"
  instances_count           = var.vm_single_count2
