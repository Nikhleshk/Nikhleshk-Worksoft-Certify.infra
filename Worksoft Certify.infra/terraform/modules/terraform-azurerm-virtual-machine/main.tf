locals {
  ...
  # generate VM names from base name and index
  # note, shortening also Linux names to 15 chars..
	instances = [for s in range(var.instances_count) : substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), s +1 ), 0, 15)]
	    
	
	nb_disks_per_instance = length(var.data_disks)
	
	vm_datadiskdisk_count_map = { for k in toset(local.instances) : k => local.nb_disks_per_instance }
    ...
    for vm_name, count in local.vm_datadiskdisk_count_map : [
      for i in range(count) : {
        ...
        lun           = i
     		disk_size_gb  		 = var.data_disks[i].disk_size_gb
    		storage_account_type = var.data_disks[i].storage_account_type
      }
    ]
  ])
...
data "azurerm_virtual_network" "vnet" {    
  name                  = element(split("/", var.existing_vnet_id), 8)
  resource_group_name   = element(split("/", var.existing_vnet_id), 4)  
}

data "azurerm_subnet" "snet" {  
  virtual_network_name  = data.azurerm_virtual_network.vnet.name
  name                  = element(split("/", var.existing_subnet_id), 10)
  resource_group_name   = element(split("/", var.existing_subnet_id), 4)   
}
...
resource "random_password" "passwd" {
  ...
  keepers = {
    admin_password = substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  }
}
...
resource "azurerm_key_vault_secret" "adminsecret" {
  ...
  name                          = var.instances_count == 1 ? substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index +1), 0, 15) : substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
...
resource "azurerm_network_interface" "nic" {
  count                         = var.instances_count
  name                          = var.instances_count == 1 ? lower("nic-${format("vm%s", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")))}") : lower("nic-${format("vm%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1)}")
  resource_group_name           = data.azurerm_resource_group.rg.name
  location                      = data.azurerm_resource_group.rg.location
  ...
  tags                          = merge({ "resourcename" = var.instances_count == 1 ? lower("nic-${format("vm%s", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")))}") : lower("nic-${format("vm%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1)}") }, var.tags, )

  ip_configuration {
    name                          = lower("ipconfig-${format("vm%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1)}")
    primary                       = true
...
resource "azurerm_linux_virtual_machine" "linux_vm" {
  ...
  name                            = var.instances_count == 1 ? substr(var.virtual_machine_name, 0, 64) : substr(format("%s%03d", lower(replace(substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15), "/[[:^alnum:]]/", "")), count.index + 1), 0, 64)
  resource_group_name             = data.azurerm_resource_group.rg.name
  location                        = data.azurerm_resource_group.rg.location
  ...
  tags                            = merge({ "resourcename" = var.instances_count == 1 ? substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15) : format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1) }, var.tags, )
...
resource "azurerm_windows_virtual_machine" "win_vm" {
  ...
  name                         = var.instances_count == 1 ? substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index +1 ), 0, 15) : substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  computer_name                = var.instances_count == 1 ? substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index +1 ), 0, 15) : substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)
  resource_group_name          = data.azurerm_resource_group.rg.name
  location                     = data.azurerm_resource_group.rg.location
  size                         = var.virtual_machine_size
  admin_username               = var.admin_username
  admin_password               = var.admin_password == null ? data.azurerm_key_vault_secret.adminsecret[count.index].value : var.admin_password
  network_interface_ids        = [element(concat(azurerm_network_interface.nic.*.id, [""]), count.index)]
  source_image_id              = var.source_image_id != null ? var.source_image_id : null
  provision_vm_agent           = true
  allow_extension_operations   = true
  ...
  tags                         = merge({ "resourcename" = var.instances_count == 1 ? substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15) : format("%s%03d", lower(replace(substr(format("%s%03d", lower(replace(substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15), "/[[:^alnum:]]/", "")), count.index + 1), 0, 15), "/[[:^alnum:]]/", "")), count.index + 1) }, var.tags, )
...
resource "azurerm_managed_disk" "data_disk" {  
  ...
  resource_group_name  = data.azurerm_resource_group.rg.name
  location             = data.azurerm_resource_group.rg.location
  storage_account_type = local.datadisk_lun_map[index(local.datadisk_lun_map[*].datadisk_name, each.key)].storage_account_type  
  #storage_account_type = lookup(each.value.data_disk, "storage_account_type", "StandardSSD_LRS")
  create_option        = "Empty"
  disk_size_gb         = local.datadisk_lun_map[index(local.datadisk_lun_map[*].datadisk_name, each.key)].disk_size_gb
...
resource "azurerm_virtual_machine_data_disk_attachment" "data_disk" {
  ...
  virtual_machine_id = var.os_flavor == "windows" ? azurerm_windows_virtual_machine.win_vm[substr(element(split("_", each.key), 1),-3,-3)-1].id : azurerm_linux_virtual_machine.linux_vm[substr(element(split("_", each.key), 1),-3,-3)-1].id
...
resource "azurerm_virtual_machine_extension" "omsagentwin" {
  ...
  name                       = var.instances_count == 1 ? "OmsAgentForWindows" : format("%s%03d", "OmsAgentForWindows", count.index + 1)
...
resource "azurerm_virtual_machine_extension" "omsagentlinux" {
  ...
  name                       = var.instances_count == 1 ? "OmsAgentForLinux" : format("%s%03d", "OmsAgentForLinux", count.index + 1)
...
resource "azurerm_monitor_diagnostic_setting" "nsg" {
  ...
  name                       = lower("nsg-${substr(format("%s%03d", lower(replace(var.virtual_machine_name, "/[[:^alnum:]]/", "")), count.index + 1), 0, 15)}-diag")
